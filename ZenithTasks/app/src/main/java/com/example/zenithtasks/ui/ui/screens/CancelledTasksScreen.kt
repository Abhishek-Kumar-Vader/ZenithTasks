...
fun CancelledTasksScreen(
    ...
    val tasksForStatus by taskViewModel.cancelledTasks.collectAsState()
    val filterPriority by taskViewModel.cancelledFilterPriority.collectAsState()
        ...
        topBar = {
            CommonTaskScreenTopAppBar(
                ...
                onFilterPrioritySelected = { priority ->
                    taskViewModel.setCancelledFilterPriority(priority)
                },
                showSearchIcon = true
            )
        },
        ...
        Box(
            ...
            Column(
                ...
                if (tasksForStatus.isEmpty()) {
                    EmptyStateVisuals(status = TaskStatus.CANCELLED)
                } else {
                    LazyColumn(
                        modifier = Modifier.fillMaxSize(),
...
@Composable
fun CancelledTasksScreenPreview() {
    ZenithTasksTheme {
        CancelledTasksScreen(navController = rememberNavController())
    }
}